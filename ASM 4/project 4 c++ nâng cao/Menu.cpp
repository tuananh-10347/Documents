...
void Menu::Menu2() //menu thông báo cài đặt từng loại thông tin
{
    ...
    do
    {
        ...
        if (number == "1")
        {
            ...
            per.replaceNodeKey(pNodeKey, temp, thongbao);//so sánh để biết rằng xe đã tồn tại hay chưa, nếu tồn tịa thì thay thế - theo Key
            per.replaceNodeName(pNodeName, temp, thongbao);//so sánh để biết rằng xe đã tồn tại hay chưa, nếu tồn tịa thì thay thế - theo Name
            
            per.insertNodeKey(pNodeKey, temp);//thêm temp vào cây MSCN
            per.insertNodeName(pNodeName, temp); // thêm name vào cây tên
    ...
    try
    {
        ...
        else if (settingInput.is_open())
        {
            while (!settingInput.eof())
            {
                ...
                per.insertNodeKey(pNodeKey,temp);
                per.insertNodeName(pNodeName, temp);
            }
            settingInput.close();
        }
    }
